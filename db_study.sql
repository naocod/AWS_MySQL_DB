/*
    ...
    3. 쿼리에디터 전체 실행 - > Ctrl + Chift + Enter
*/

insert into customer_mst values (1, '나오', '부산 동래구 사직동');
insert into customer_mst values (2, '다오', '부산 부산진구 가야동');
insert into customer_mst values (3, '가오', '부산 남구 대연동');

select * from customer_mst; -- 주문자 테이블 조회
...
select * from product_mst; -- 상품 테이블 조회

insert into order_mst values (1, 23011901, 1); -- pk, 주문번호, 고객번호
...
insert into order_dtl values (1, 1, 1); -- 1번째 주문에서 1번 상품주문
insert into order_dtl values (2, 1, 2); -- 1번째 주문에서 2번 상품주문
